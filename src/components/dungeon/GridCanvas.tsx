    ...
    for (let r = 0; r < height; r++) {
      for (let c = 0; c < width; c++) {
        ...
        for (const [dr, dc] of dirs4) {
          const nr = r + dr,
            nc = c + dc;
          if (nr >= 0 && nr < height && nc >= 0 && nc < width && cells[nr][nc] === 1 && !wallVis[nr][nc]) {
            toMark.push([nr, nc]);
          }
        }
      }
...
function drawWallHatching(
  ...
  const points = poissonDiskWall(pxW, pxH, HATCH_POISSON_SPACING * RENDER_SCALE, wallVis, cells, rng);
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    for (let dy = -1; dy <= 1; dy++) {
      ...
      for (let dx = -1; dx <= 1; dx++) {
        ...
        for (const ai of angleBuckets[bry * bCols + brx]) {
          ...
          if (d2 < nearestD2) { nearestD2 = d2; nearest = nb; }
        }
      }
    }
    ...
    if (nearest) {
      ...
      const off = HATCH_NEIGHBOR_ANGLE_MIN + rng() * (HATCH_NEIGHBOR_ANGLE_MAX - HATCH_NEIGHBOR_ANGLE_MIN);
    ...
    // --- Length: Voronoi-fit ---
    const fitLen = Math.max(HATCH_LENGTH_MIN, Math.min(HATCH_LENGTH_MAX,
      nnDist[i] * HATCH_VORONOI_FILL / RENDER_SCALE)) * RENDER_SCALE;
    const len = fitLen * (HATCH_LENGTH_RAND_MIN + rng() * (HATCH_LENGTH_RAND_MAX - HATCH_LENGTH_RAND_MIN));
      ...
      for (let r = 0; r < height; r++) {
        for (let c = 0; c < width; c++) {
          ...
          for (const [dr, dc] of dirs4) {
            const nr = r + dr,
              nc = c + dc;
            if (nr >= 0 && nr < height && nc >= 0 && nc < width && cells[nr][nc] === 0 && !corVisible[nr][nc]) {
              corVisible[nr][nc] = 1;
              q.push([nr, nc]);
            }
          }
