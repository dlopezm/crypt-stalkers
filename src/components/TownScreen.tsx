...
export function TownScreen({ player, onUpdatePlayer, onEnterDungeon }: {
  player: Player;
  ...
  function buyWeapon(w: Weapon) {
    ...
    onUpdatePlayer({ ...player, gold: player.gold - w.cost, weapons: [...player.weapons, { ...w }] });
  }

  function buyConsumable(c: Consumable) {
    if (player.gold < c.cost) return;
    onUpdatePlayer({ ...player, gold: player.gold - c.cost, consumables: [...player.consumables, { ...c }] });
  }
  ...
  function unlockBuilding(b: BuildingDef) {
    ...
    onUpdatePlayer({ ...player, gold: player.gold - b.unlockCost, buildings, abilities: newAbilities });
  }
  ...
  function upgradeBuilding(b: BuildingDef) {
    ...
    onUpdatePlayer({ ...player, gold: player.gold - b.upgradeCost, buildings, abilities: newAbilities });
  }
    ...
    if (!state?.unlocked) {
      return (
        <div className="panel max-w-lg w-full">
          <div className="text-lg font-bold text-crypt-text mb-2">{b.icon} {b.name}</div>
          <p className="text-sm text-crypt-muted mb-4">{b.desc}</p>
          <p className="text-sm text-crypt-dim mb-3">This building is locked.</p>
          <button style={btnStyle("#8b0000", player.gold < b.unlockCost)} disabled={player.gold < b.unlockCost}
            onClick={() => unlockBuilding(b)}>
            Unlock ({b.unlockCost}{"\u{1FA99}"})
          </button>
        </div>
      );
    }
  ...
  function renderSmithy() {
    ...
    return (
      <div className="panel max-w-xl w-full">
        ...
        <p className="text-xs text-crypt-dim mb-3">Purchase weapons. You can switch between owned weapons in combat.</p>
        <div className="flex flex-col gap-2">
          {WEAPONS.filter(w => w.cost > 0).map(w => (
            <div key={w.id} className="flex justify-between items-center pb-2 border-b border-crypt-border-dim">
              <div className="flex-1">
                <div className="text-sm text-crypt-text">{w.icon} {w.name} <span className="text-crypt-dim">({w.damage} {w.range})</span></div>
                <div className="text-xs text-crypt-muted">{w.desc}</div>
              </div>
              {owned.has(w.id)
                ? <span className="text-xs text-crypt-green ml-3">Owned</span>
                : <button style={btnStyle("#8b0000", player.gold < w.cost)} disabled={player.gold < w.cost}
                    className="ml-3 whitespace-nowrap" onClick={() => buyWeapon(w)}>
                    {w.cost}{"\u{1FA99}"}
                  </button>
              }
            </div>
  ...
  function renderStore() {
    return (
      <div className="panel max-w-xl w-full">
        ...
        <div className="flex flex-col gap-2">
          {CONSUMABLES.map(c => (
            <div key={c.id} className="flex justify-between items-center pb-2 border-b border-crypt-border-dim">
              <div className="flex-1">
                <div className="text-sm text-crypt-text">{c.icon} {c.name}</div>
                <div className="text-xs text-crypt-muted">{c.desc}</div>
              </div>
              <button style={btnStyle("#8b0000", player.gold < c.cost)} disabled={player.gold < c.cost}
                className="ml-3 whitespace-nowrap" onClick={() => buyConsumable(c)}>
                {c.cost}{"\u{1FA99}"}
              </button>
            </div>
          ))}
        ...
        {player.consumables.length > 0 && (
          <div className="mt-3 pt-3 border-t border-crypt-border-dim">
            <div className="text-xs text-crypt-dim mb-1">YOUR INVENTORY ({player.consumables.length})</div>
            <div className="flex gap-1 flex-wrap">
              {player.consumables.map((c, i) => (
                <span key={i} className="text-xs bg-[#1a1610] border border-crypt-border-dim rounded px-2 py-0.5 text-crypt-muted">
                  {c.icon} {c.name}
                </span>
              ))}
            </div>
    ...
    return (
      <div className="panel max-w-lg w-full">
        ...
        <p className="text-sm text-crypt-muted mb-3">
          The barkeep nods toward the back of the tavern. A cloaked figure traces the outline of a map.
        </p>
        <p className="text-xs text-crypt-dim italic">
          "Three paths into the dark, stranger. Shallow Graves for the green. The Crypt for the tested. The Lich's Domain... for the doomed."
        </p>
      </div>
    );
  }
  ...
  function renderAbilityBuilding(b: BuildingDef) {
    ...
    return (
      <div className="panel max-w-xl w-full">
        <div className="text-lg font-bold text-crypt-text mb-1">{b.icon} {b.name} <span className="text-xs text-crypt-dim">(Level {state?.level || 0})</span></div>
        ...
        <div className="flex flex-col gap-2">
          {buildingAbilities.map(a => {
            ...
            return (
              <div key={a.id} className={`flex justify-between items-center pb-2 border-b border-crypt-border-dim ${!available ? "opacity-40" : ""}`}>
                <div className="flex-1">
                  <div className="text-sm text-crypt-text">{a.icon} {a.name}</div>
                  <div className="text-xs text-crypt-muted">{a.desc}</div>
                  {!available && <div className="text-xs text-crypt-red">Requires building level {a.buildingLevel}</div>}
                </div>
        ...
        {state && state.level < 2 && b.upgradeCost > 0 && (
          <button style={btnStyle("#6a3a1a", player.gold < b.upgradeCost)} disabled={player.gold < b.upgradeCost}
            className="mt-3" onClick={() => upgradeBuilding(b)}>
            Upgrade to Level 2 ({b.upgradeCost}{"\u{1FA99}"})
          </button>
        )}
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-crypt-bg text-crypt-text font-serif flex flex-col items-center gap-4 relative overflow-y-auto p-4">
      ...
      <div className="relative z-1 text-center">
        <h1 className="text-[clamp(1.8rem,4vw,2.8rem)] tracking-[0.15em] uppercase text-crypt-gold font-bold"
          style={{ textShadow: "0 0 20px #f0c040, 0 0 40px #8a6010" }}>
          {"\u{1F3F0}"} Town
        </h1>
        <div className="text-crypt-dim tracking-[0.25em] text-xs">{"\u25C6"} A HAVEN FROM DARKNESS {"\u25C6"}</div>
      </div>
      ...
      <div className="panel max-w-2xl w-full relative z-1">
        <div className="flex gap-6 items-center flex-wrap justify-center">
          ...
          <div className="text-crypt-gold text-base font-bold">{"\u{1FA99}"} {player.gold}</div>
          <div className="text-crypt-muted text-sm">{"\u{1F5E1}\uFE0F"} {player.weapons.length} weapons</div>
          <div className="text-crypt-muted text-sm">{"\u{1F392}"} {player.consumables.length} items</div>
          {player.abilities.length > 0 && (
            <div className="text-crypt-muted text-sm">{"\u2728"} {player.abilities.length} abilities</div>
          )}
        </div>
        ...
        {BUILDINGS.map(b => {
          ...
          return (
            <div key={b.id}
              ...
              <div className="text-sm font-bold text-center text-crypt-text leading-tight mb-1">{b.name}</div>
              {unlocked
                ? <div className="text-xs text-crypt-dim text-center">{state?.level ? `Level ${state.level}` : ""}</div>
                : <div className="text-xs text-crypt-red text-center">{b.unlockCost}{"\u{1FA99}"} to unlock</div>
              }
            </div>
          );
        })}
      ...
      {!activeBuilding && (
        <div className="relative z-1 w-full max-w-2xl">
          <div className="text-xs text-crypt-dim tracking-[0.2em] mb-2 uppercase text-center">{"\u2620"} Choose Your Descent {"\u2620"}</div>
          <div className="flex gap-3 flex-wrap justify-center">
            {DUNGEONS.map(d => (
              <div key={d.id} className="panel flex flex-col items-center gap-2" style={{ width: "200px" }}>
                ...
                <div className="text-xs text-crypt-dim">{d.slots.filter(s => s.type === "combat").length} combat {"\u00B7"} {d.slots.filter(s => s.type === "rest").length} rest</div>
                <button style={btnStyle("#8b0000")} className="w-full" onClick={() => onEnterDungeon(d)}>
                  {"\u2694"} Descend
                </button>
              </div>
            ))}
      ...
      {!activeBuilding && (
        <div className="panel max-w-2xl w-full relative z-1">
          <div className="text-xs text-crypt-dim tracking-[0.2em] mb-2 uppercase">Equipment & Inventory</div>
          <div className="flex flex-col gap-2">
            <div>
              <div className="text-xs text-crypt-muted mb-1">Weapons:</div>
              <div className="flex gap-1 flex-wrap">
                {player.weapons.map((w, i) => (
                  <span key={i} className={`text-xs rounded px-2 py-0.5 ${i === player.activeWeaponIdx ? "bg-[#3a2a14] border border-crypt-gold text-crypt-gold" : "bg-[#1a1610] border border-crypt-border-dim text-crypt-muted"}`}>
              ...
              <div>
                ...
                <div className="flex gap-1 flex-wrap">
                  {player.abilities.map(id => {
                    ...
                    return a ? (
                      <span key={id} className="text-xs bg-[#1a1610] border border-crypt-border-dim rounded px-2 py-0.5 text-crypt-muted">
                        {a.icon} {a.name}
                      </span>
                    ) : null;
                  })}
